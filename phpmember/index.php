...
if (isset($_POST["username"]) && isset($_POST["passwd"])) {
	...
	if (password_verify($_POST["passwd"], $passwd)) {
		...
		if (isset($_POST["rememberme"]) && ($_POST["rememberme"] == "true")) {
			setcookie("remUser", $_POST["username"], time() + 365 * 24 * 60*60);
			//setcookie("remPass", $_POST["passwd"], time() + 365 * 24 * 60*60);
		} else {
			if (isset($_COOKIE["remUser"])) {
				setcookie("remUser", $_POST["username"], time() - 100);//取消勾選記住我的帳號密碼
		...
		header("Location: index.php?errMsg=1");//帳比對成但密碼錯誤則跳到index.php且傳URL參數errMsg=1
	}
}
?>
<html>
            ...
            <td width="200">
              ...
              <div class="regbox"><?php if (isset($_GET["errMsg"]) && ($_GET["errMsg"] == "1")) {?>
                ...
                <form name="form1" method="post" action="">
                  <p>帳號：
                    <br>
                    <input name="username" type="text" class="logintextbox" id="username" value="<?php if (isset($_COOKIE["remUser"]) && ($_COOKIE["remUser"] != "")) {
	echo $_COOKIE["remUser"];
}
?>">
                  </p>
                  <p>密碼：<br>
                    <input name="passwd" type="password" class="logintextbox" id="passwd" value="<?php if (isset($_COOKIE["remPass"]) && ($_COOKIE["remPass"] != "")) {
